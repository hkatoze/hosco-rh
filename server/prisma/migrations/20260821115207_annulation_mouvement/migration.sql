-- AlterTable
ALTER TABLE "Mouvement" ADD COLUMN     "annuleLe" TIMESTAMP(3),
ADD COLUMN     "annuleParId" TEXT,
ADD COLUMN     "motifAnnulation" TEXT;

-- AddForeignKey
ALTER TABLE "Mouvement" ADD CONSTRAINT "Mouvement_annuleParId_fkey" FOREIGN KEY ("annuleParId") REFERENCES "Utilisateur"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- Mise à jour de la vue AgentStatutCourant (miroir de server/src/domain/statut.ts,
-- voir migration 20260820183144_vue_statut_agent) : un mouvement annulé
-- (annuleLe non nul) est désormais ignoré, comme s'il n'existait pas.
CREATE OR REPLACE VIEW "AgentStatutCourant" AS
WITH applicables AS (
  SELECT *
  FROM "Mouvement"
  WHERE "dateEffet" <= CURRENT_DATE
    AND "annuleLe" IS NULL
),
definitifs AS (
